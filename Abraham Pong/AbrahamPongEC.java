...
public class AbrahamPongEC {
	...
	public static boolean ballLeft; // boolean if the ball is moving left or
									// right
	...
	public static void startUp() {
		...
		// randomize initial ball location and movement of up or down y direction
		ballX = ThreadLocalRandom.current().nextInt(300, 477);
		ballY = ThreadLocalRandom.current().nextDouble(200, 350);
		int upOrDown = ThreadLocalRandom.current().nextInt(0, 10);
		if (upOrDown >= 6) {
			ballUp = true;
			ballDown = false;
		} else if (upOrDown <= 4) {
	...
	public static void startGame() {
		...
		while (gameInAction == true) {
			...
			if (ballX > 35 && (ballLeft == true)) { // if ball x is over 35 and
				...
				if (ballUp == true) { // check if ball should be going up or
										// down
	...
	public static void reDraw() { // this method redraws the game according to
		...
		if (ballY < 30 && ballY > 0) { // if ball hits the bottom, change Y
										// direction

			ballDown = false;
			ballUp = true;
		}
		if (ballY > 480 && ballY < 512) { // if ball hits the top, change Y
											// direction
