...
public class AbrahamPong4Players {

	public static double leftPaddleY; // Y value of the left paddle
	public static double rightPaddleY; // Y value of the right paddle
	public static double bottomPaddleX; //X value of the bottom paddle
	public static double topPaddleX; //X value of the top paddle
	...
	public static void startUp() {
		...
		StdDraw.filledRectangle(bottomPaddleX, 10, 40, 10);
		StdDraw.filledRectangle(topPaddleX, 502, 40, 10);
	...
	public static void startGame() {
		...
		while (gameInAction == true) {
			...
			if (ballX < 477 && (ballLeft == false)) { // if ball x is under 477
			 ...
			 if ((ballY < 30) && (ballX) < (bottomPaddleX - 60)){
				quit(); // if ball hits the wall, end the game
			}
			 if ((ballY < 30) && (bottomPaddleX + 60) < (ballX) ){
				quit(); // if ball hits the wall, end the game
			}
			 if ((ballY > 480) && (ballX) < (topPaddleX - 60)){
				quit(); // if ball hits the wall, end the game
			}
			 if ((ballY > 480) && (topPaddleX + 60) < (ballX) ){
				quit(); // if ball hits the wall, end the game
			}
	...
	public static void checkKey() { // method to check which key was pressed
		...
		// if the key pressed was v, and the x of the bottom paddle is above 40,
		// move paddle left
		else if ((key) == 'v') {
			if (bottomPaddleX > 41) {
				bottomPaddleX = bottomPaddleX - paddleSpeed;
				reDraw();
			}
		}
		// if the key pressed was b, and the x of the bottom paddle is below 472,
		// move paddle right
		else if ((key) == 'b') {
			if (bottomPaddleX < 471) {
				bottomPaddleX = bottomPaddleX + paddleSpeed;
				reDraw();
			}
		}
		// if the key pressed was t, and the x of the top paddle is above 40,
				// move paddle left
				else if ((key) == 't') {
					if (topPaddleX > 41) {
						topPaddleX = topPaddleX - paddleSpeed;
						reDraw();
					}
				}
				// if the key pressed was y, and the x of the top paddle is below 472,
				// move paddle right
				else if ((key) == 'y') {
					if (topPaddleX < 471) {
						topPaddleX = topPaddleX + paddleSpeed;
	...
	public static void reDraw() { // this method redraws the game according to
		...
		// if ball hits the bottom paddle, change Y direction
		if ((ballY < 30 && ballY > 0) && ((bottomPaddleX - 60) < (ballX)) && (ballX) < (bottomPaddleX + 60)) { 	
			ballDown = false;
			ballUp = true;
			//check if the ball hit the left of right side of paddle
			//if left, send the ball to the left
			//if right, send the ball to the right
			if ((bottomPaddleX) < (ballX) && (ballX) < (bottomPaddleX + 60)) {
				ballLeft = false;
			} else if ((bottomPaddleX - 60) < (ballX) && (ballX) < (bottomPaddleX)) {
				ballLeft = true;
			}
		}
		// if ball hits the top paddle, change Y direction
		if ((ballY > 480 && ballY < 512) && ((topPaddleX - 60) < (ballX)) && (ballX) < (topPaddleX + 60)) { 
			...
			//if left, send the ball to the left
			//if right, send the ball to the right
			if ((topPaddleX) < (ballX) && (ballX) < (topPaddleX + 60)) {
				ballLeft = false;
			} else if ((topPaddleX - 60) < (ballX) && (ballX) < (topPaddleX)) {
				ballLeft = true;
			}
		}
	    ...
	    if (ballX == 477 && ((rightPaddleY - 60) < (ballY)) && (ballY) < (rightPaddleY + 60)) {
			// check if the ball is hitting the top or bottom of the paddle
			// if it hits the top set the ball to go up, if it hit the bottom of
			// the paddle set the ball to go down
